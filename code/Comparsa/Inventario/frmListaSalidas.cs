...
namespace Comparsa
{
    public partial class frmListaSalidas : Form
    {
        ...
        private static frmListaSalidas Instance = null;
        ...
        public static void ShowAsMDIChild(Form mdiParent)
        {
            if (Instance == null)
            {
                Instance = new frmListaSalidas { MdiParent = mdiParent };
                Instance.Show();
            }
            else
            {
        ...
        private void frmListaSalidas_FormClosed(object sender, FormClosedEventArgs e)
        {
            SaveWindowConfig();
            Instance = null;
        }
        ...
        private void LoadGridData()
        {
            ...
            using (var db = Globals.DataContext.CreateDataConnection())
            {
                ...
                var query = (
                    from s in db.GetTable<SALIDA>()
                    join r in db.GetTable<COLABORADOR>() on s.RESPONSABLEID equals r.COLABORADORID
                    join l in db.GetTable<LOCALIDAD>() on s.LOCALIDADID equals l.LOCALIDADID
                    select new SALIDA_EXT
                    {
                        SALIDAID = s.SALIDAID,
                        NUMERO = s.NUMERO,
                        FECHA = s.FECHA,
        ...
        private bool BorrarRegistro()
        {
            ...
            if (gridView.SelectedRows.Count > 0)
            {
                ...
                registro = (SALIDA)row.DataBoundItem;
        ...
        private bool InfoRegistro(CRUDMode mode)
        {
            ...
            frmDetSalida frmDetSalida = new frmDetSalida();
            frmDetSalida.mode = mode;
            if (mode == CRUDMode.Update)
            {
                frmDetSalida.registro = registro;
            }
            dr = frmDetSalida.ShowDialog();
            frmDetSalida.Dispose();
            frmDetSalida = null;
        ...
        private void frmListaSalidas_Resize(object sender, EventArgs e)
        {
